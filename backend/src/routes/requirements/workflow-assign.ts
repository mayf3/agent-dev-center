...
export function registerWorkflowAssignRoutes(router: import('express').Router): void {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const snapshotSteps = JSON.parse(JSON.stringify(template.steps));
      const stepsArray = Array.isArray(snapshotSteps) ? snapshotSteps : (snapshotSteps as any)?.steps ?? [];
      const extracted = extractRoleUserMap(template.steps);
      const workflowSnapshot = extracted
        ? { steps: stepsArray, roleUserMap: extracted }
        : stepsArray;
