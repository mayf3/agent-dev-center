...
describe('POST /:id/workflow/reject — assignee routing', () => {
  ...
  it('lock path (deploying → dev_self_check) success: findUnique + delete + update + transition in order', async () => {
    ...
    expect(res.status).toBe(200);
    // lock findUnique called once to build ownership (release is atomic deleteMany)
    ...
    expect(mockLockDeleteMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ id: 'singleton' }) }),
    );

    // 完整调用顺序：findUnique < deleteMany < update < transition
