...
/**
 * 获取完整的工作流原始 JSON（含 roleUserMap）
 * 优先使用 snapshot（新的对象格式），旧数组格式会尝试从模板表补充 roleUserMap
 */
export async function getWorkflowRawJsonWithRoleUserMap(
  requirement: {
    workflowSnapshot?: unknown;
    workflowId?: string | null;
    workflow?: { steps: unknown; roleUserMap?: unknown } | null;
  } | null | undefined,
): Promise<unknown> {
  if (!requirement) return null;

  // 新 snapshot 格式：{ steps: [...], roleUserMap: {...} } — 直接使用
  const snapshot = requirement.workflowSnapshot;
  if (snapshot !== null && snapshot !== undefined) {
    if (!Array.isArray(snapshot) && 'steps' in (snapshot as Record<string, unknown>)) {
      return snapshot;  // 新格式，已有 roleUserMap
    }
    // 旧格式：纯数组 → 尝试从模板表补充 roleUserMap
    if (Array.isArray(snapshot) && requirement.workflowId) {
      try {
        const { prisma } = await import('../../lib/prisma.js');
        const template = await prisma.workflowTemplate.findUnique({
          where: { id: requirement.workflowId },
          select: { steps: true },
        });
        const rum = template ? extractRoleUserMap(template.steps) : undefined;
        if (rum) return { steps: snapshot, roleUserMap: rum };
      } catch {
        // 静默失败，返回原 snapshot
      }
    }
