...
| **Folder V1** | 后端实体 = `BusinessDomain`，API 字段 = `domainKey` |
...
- **API 响应**: `folder` / `folderKey`（通过 serializer 映射）
...
两个字段是唯一真源，存储在 `requirements` 表：

| 字段 | 类型 | 说明 |
|------|------|------|
| `status` | Enum | 顶层生命周期 |
| `currentStep` | String | 工作流当前步骤（可空，纯 Todo 无工作流） |
...
|------|------|---------|
| `draft` | 草稿，尚未提交 | 创建时默认 |
| `pending` | 已提交，等待开始 | 用户提交或 Agent 创建 |
| `in_progress` | 进行中 | 被领取或工作流启动 |
| `done` | 完成 | 工作流完成或手动标记 |
| `cancelled` | 取消 | 用户取消 |
| `archived` | 归档 | 不再活跃，数据保留 |

### 2.3 派生视图（不持久化）

以下状态不存储在数据库中，由查询实时计算：
...
派生视图通过 API query parameter 实现（如 `?view=overdue`），不创建额外的 `assignmentState` 或 `deliveryPath` 字段。
...
| `snoozedUntil` | DateTime? | 延后处理时间 |
| `blockedReason` | String? | 阻塞原因 |
| `completedAt` | DateTime? | 实际完成时间 |
| `parentId` | String? @db.Uuid | 父任务 ID（V1 不实现嵌套，但预留字段） |
...
### 4.1 Todo "area" → Folder/Domain

| Todo area | Folder/Domain Key | 说明 |
|-----------|------------------|------|
| `dev` | `engineering` | 开发 |
| `ops` | `operations` | 运维 |
...
| comments | `RequirementComment`（新表，Batch 2） | 独立评论表 |
...
兼容 API 返回旧 Todo ID，通过 `legacySource` + `legacyId` 映射：

- **请求**: 兼容 API 接受旧 Todo ID
- **查找**: `WHERE legacySource = 'todo-sqlite' AND legacyId = <oldId>`
- **响应**: 同时返回 `id`（新 Requirement UUID）和 `legacyId`（旧 Todo ID）

### 6.3 幂等迁移

- `(legacySource, legacyId)` 建立唯一约束
...
### Batch 0: Migration Architecture 与 Fresh Bootstrap 修复

| 维度 | 内容 |
|------|------|
| **schema** | 修复 migration 排序；建立 `bootstrap-fresh-db.sh`；lockToken nullable UUID |
| **API** | 无 |
| **frontend** | 无 |
| **callers** | 无 |
| **tests** | Fresh DB 自举测试；existing DB 升级测试；gate test |
| **rollback** | 不需要（基础设施） |
| **审计门** | `MIGRATION_ARCHITECTURE_READY_FOR_AUDIT` |

**状态**: ✅ lockToken migration 已创建。Fresh bootstrap 脚本已创建。Migration 排序问题已记录。
...
| **API** | `GET/POST/PUT/DELETE /api/folders`；`roleAwareRequirementWhere` 增加 domain 过滤 |
| **frontend** | 无（V1 后端先行） |
| **callers** | 无 |
...
| 维度 | 内容 |
|------|------|
| **schema** | `scheduledAt`, `snoozedUntil`, `blockedReason`, `completedAt`, `parentId`, `legacySource`, `legacyId` 字段；`RequirementReminder` model；`RequirementComment` model；`(legacySource, legacyId)` 唯一约束 |
| **API** | `GET /api/requirements?view=overdue\|blocked\|review\|waiting`；reminder CRUD；comment CRUD |
| **frontend** | 无 |
| **callers** | efficiency-agent 开始对接 reminder API |
| **tests** | 派生视图查询正确性；reminder CRUD；comment CRUD；legacyId 唯一性 |
| **rollback** | 所有新字段 nullable，旧代码不受影响 |
...
|------|------|
| **schema** | 无新表 |
| **API** | Todo 兼容端点（`GET/POST/PUT/DELETE /api/todo/:legacyId` 映射到 Requirement）；旧 Todo 写端点返回 410 Gone |
| **frontend** | 无 |
...
| **tests** | 数据迁移幂等性；area → domainKey 映射正确；记录数一致；旧 API 只读验证 |
| **rollback** | 旧系统恢复写权限；已迁移数据保留（`legacySource` 标记） |
...
| **callers** | 所有调用方确认已迁移 |
| **tests** | 旧 API 返回 404；前端 Folder 视图正常；7 天无旧 API 写请求 |
...
| Comment @mention 解析 | 是否需要解析 @username 并通知 | Batch 2 实施前 |
...
model RequirementReminder {
  ...
  remindAt      DateTime
  channel       String   @default("web")  // web, email, webhook
  message       String?
  sentAt        DateTime?
  createdAt     DateTime @default(now())
...
| Status (todo) | Status (requirement) | 统一 status enum |
